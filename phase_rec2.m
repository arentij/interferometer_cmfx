...
dt_phase_red = 10e-6;    % resolution (window size)
dt_phase_co2 = 10e-6;    % resolution (window size)
...
%
driver_r = data(1:end,2);
signal_r = data(1:end,1);
% 
% driver_r = data1(1:end,3);
% signal_r = data1(1:end,2);


driver_c = data(1:end,4);
signal_c = data(1:end,3);

N_pts = size(driver_r,1);
% t_art = linspace(0,N_pts/fs,N_pts);
% t_art = t_art - t_art(end)/2;

%% rec_phase will reconstruct the phase shift array 
[rec_phase_arr_r,t_rec_phase_r] =  rec_phase_fft(driver_r,signal_r,fs,dt_phase_red,ovlp);
t_rec_phase_r = t_rec_phase_r - (t_rec_phase_r(end)-t_rec_phase_r(1))/2;

[rec_phase_arr_c,t_rec_phase_c] =  rec_phase_fft(driver_c,signal_c,fs,dt_phase_co2,ovlp);
t_rec_phase_c = t_rec_phase_c - (t_rec_phase_c(end)-t_rec_phase_c(1))/2;
%%
% figure(1000)
% 
% if clear_figs
...
% [px, fff] = pwelch(detrend(rec_phase_arr_r).*hanning(size(rec_phase_arr_r,1),"periodic") ,[],[],[],fs_rec);
...
% rec_phase_arr_ds2 = unwrap(2*rec_phase_arr_r)/2;   % let's get rid of the N*Pi shifts
...
% t_arr_ds = movmean(t_rec_phase_r,2*downsampling);  % the same with the time array
...
% acc_plt;
%%
% save([plt_name  'exp' num2str(exp_number) '/exp' num2str(exp_number) '_rec_phase.mat'],'t_rec_phase_r','rec_phase_arr_r','t_rec_phase_c','rec_phase_arr_c');
