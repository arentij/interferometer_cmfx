...
signal = data(1:4:end,1) - mean(data(1:4:end,1));
...
for fit_try = 1:10
    ...
    dN = fix(dl_fit*fs);
    Ni_chunks0 = [1, k0m];
    Ni_chunks0 = [1, k0m, k01];
    % Ni_chunks0 = [1, k0m, N_d-dN];
    ...
    fittedmdl2 = fit(time_to_fit',signal_to_fit,mdl,'start',[approx_ampl 40e6+134 phi0]);
...
plot(t_array(1:100:end),sig_dr(1:100:end)/a1,'k')
...
for j = 1:3
    for i=1:length(deconstr_sig)-1
        if (deconstr_sig(i+1) - deconstr_sig(i)) > pi*0.8
            add_per_step =  - pi;
            deconstr_sig(i+1) = deconstr_sig(i+1) + add_per_step;
        elseif (deconstr_sig(i+1) - deconstr_sig(i)) < -pi*0.8
            add_per_step =  + pi;
            deconstr_sig(i+1) = deconstr_sig(i+1) + add_per_step;
        end
    end
...
if f_found
    plot(t_array(1:500:end),lp_flt_phase(1:500:end),'r','LineWidth',2.0)
else
...
plot(fff(fft_plt_arr), log_px(fft_plt_arr))
